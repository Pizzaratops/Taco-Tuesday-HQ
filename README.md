...
7. **Trade Calculator an Matt Lawsons neues Punkte-System anpassen?** Beyaz hat zusätzlich zur Dynasty-Rangliste Matt Lawsons "NBA Dynasty Trade Calculator" (Categories) hochgeladen — ein Punktesystem (Spieler + Picks → Zahlenwert, z.B. Wembanyama 1590, 2026 #1-2 Pick 770), deutlich reichhaltiger als der bisherige rein rangbasierte Ansatz in `js/trade-analyzer.js`. Offene Design-Frage, noch nicht umgesetzt: als zusätzlichen 4. Modus neben Dynasty/Raw/Win-Now einbauen, oder den bestehenden Rang-Ansatz komplett ersetzen? Datei liegt bereit, aber noch nicht integriert.
8. **NBA Trades reaktivieren, sobald eine kostenlose Alternative zu Balldontlie gefunden ist** — aktuell pausiert (siehe "Zuletzt gemacht"), UI zeigt ehrlichen Hinweis statt totem Button.
9. **Aufräumen:** doppelte `draft-capital-2026.js` (liegt sowohl in `scripts/` als auch `data/` — nur die Version in `data/` wird gebraucht), eine verrutschte `daily-9cat.js` in `.github/workflows/` (gehört nach `scripts/`, liegt dort auch schon korrekt), sowie drei weitere Verrutscher in `js/`: `build-postdraft-board.js` (gehört nach `scripts/`), `draft2026.js` und `draft2027.js` (gehören nach `data/`) — alle drei sind identische Kopien der korrekten Dateien, können einfach aus `js/` gelöscht werden. ~88 Spieler in `data/rankings.js` (v.a. tiefe 2026er Draft-Picks) haben noch keine Positions-Angabe, da sie weder in der xlsx noch im Pre-Draft Big Board mit Position auftauchen.
